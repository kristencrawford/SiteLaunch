...
if [ ${ACCT} == "" ]; then
  echo "`date +"%c"`: The ${ENV} Alias is missing from ${SITE_ID}.json. Web build cannot be completed without this information. Please ensure create subaccount finished successfully and updated the site's json."
  exit 1
fi
...
function createWebserver {
  ...
  if [[ ${groupID} != "" && ${networkID} != "" ]]; then
    webJSON="{\"name\":\"web${1}\",\"description\":\"web server\",\"groupId\":\"${groupID}\",\"sourceServerId\":\"VA1TCCCTMPL02\",\"isManagedOS\":false,\"primaryDns\":\"172.17.1.26\",\"networkId\":\"${networkID}\",\"cpu\":1,\"memoryGB\":2,\"type\":\"standard\",\"storageType\":\"standard\",\"Packages\":[{\"packageID\":\"24485861-e860-48bd-83c2-2bacb167f3ec\"}]}"

    # Build Webserver
    build=`curl -s "https://${ENDPOINT}/v2/servers/${ACCT}" -XPUT -H "Authorization: Bearer ${TOKEN}" -XPOST -H "Content-type: application/json" -d "${webJSON}"`
...
function updateJSONWeb {
  ...
  if [ -z "${server_found}" ]; then
    ...
    newJSON=`jq '(.Environments[] | select(.Name=="'${ENV}'") | .Servers['${total}']) |= { "Name" : "'${1}'","Type" : "web","Middleware" : "_NewServer_" }' ./${SITE_ID}.json`
    rm -rf ./${SITE_ID}.json
...
if [ ${found} -eq 0 ]; then
  if [ ${REF_ARCH} == "Basic" ]; then
    basicRequestID=$(createWebserver 1);
    
    if [ ${basicRequestID} -ne 0 ]; then
      echo "`date +"%c"`: Build webserver on ${ACCT} has been queued. You will get an update every 30 seconds."
	  ...
	  getAuth;
        ...
        basicServer=`echo ${getStatus} | jq -r .Servers | grep WEB1 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
	...
	exit 1
      ...
      echo "`date +"%c"`: Build web server failed to run..  Either the blueprint failed to finish or the network or group have not been created! Bailing..."
      exit 1
    fi

  elif [ ${REF_ARCH} == "Standard" ]; then
    standardReqID1=$(createWebserver 1);
    standardReqID2=$(createWebserver 2);
    if [[ ${standardReqID1} -gt 2 && ${standardReqID2} -gt 2 ]]; then
      echo "`date +"%c"`: Build web standard servers on ${ACCT} have been queued. You will get an update every minute."
      ...
      while [ ${buildPercent1} -lt 100 ]; do
        while [ ${buildPercent2} -lt 100 ]; do
          ...
          standardServer1=`echo ${getBuildStatus1} | jq -r .Servers | grep WEB1 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
          standardServer2=`echo ${getBuildStatus2} | jq -r .Servers | grep WEB2 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
      ...
      # Update JSON
      updateJSONWeb ${standardServer1};
      updateJSONWeb ${standardServer2};
	...
	echo "`date +"%c"`: Servers ${standardServer1} and ${standardServer2} are completed and ready for use."
      ...
      echo "`date +"%c"`: Build standard web servers for ${ACCT} failed to run.. Either the blueprints failed to finish or the network or group have not been created! Bailing..."
      exit 1
    fi
  else
  ...
  if [ ${REF_ARCH} == "Basic" ]; then
    echo "`date +"%c"`: The required Basic ${ENV} Web Servers for ${ACCT} is built. Moving on..."
  elif [ ${REF_ARCH} == "Standard" ]; then
    basicRequestID=$(createWebserver 2);
    if [ ${basicRequestID} -ne 0 ]; then
      echo "`date +"%c"`: Build webserver on ${ACCT} has been queued. You will get an update every 30 seconds."
	  ...
	  getAuth;
        ...
        basicServer=`echo ${getStatus} | jq -r .Servers | grep WEB2 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
        echo "`date +"%c"`: Server ${basicServer} for ${ACCT} build is ${basicBuildPercent}% complete..."
        timer=$(( timer + 1 ))
        if [ ${timer} == 20 ]; then
          break
  ...
  echo "`date +"%c"`: The required ${ENV} Web Servers for ${ACCT} are built. Moving on..."
fi
