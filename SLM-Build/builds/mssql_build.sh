...
if [ ${ACCT} == "" ]; then
  echo "`date +"%c"`: The ${ENV} Alias is missing from ${SITE_ID}.json. MsSql build cannot be completed without this information. Please ensure create subaccount finished successfully and updated the site's json."
  exit 1
fi
...
function getMSSQLServers {
  # Check mssql server total before creating any servers
  hardwareGroupID=`curl -s "https://${ENDPOINT}/REST/Group/GetGroups/JSON" -XPOST -H "Content-type: application/json" -b "cookies.txt" -d "{\"AccountAlias\":\"${ACCT}\",\"Location\":\"${DC}\"}" | jq '.HardwareGroups[] | select(.Name=="'${SITE_ID}' - '${ENV}' - DB") | .UUID' | sed "s/\"//g"`
...
function createMSSQLServer {
  getAuth;
  # Create build mssql server json
  ...
  groupID=`echo ${getGroups} | jq '.HardwareGroups[] | select(.Name=="'${SITE_ID}' - '${ENV}' - DB") | .UUID' | sed "s/\"//g"`
  getNetworks=`curl -s "https://${ENDPOINT}/v2-experimental/networks/${ACCT}/${DC}" -XGET -H "Authorization: Bearer ${TOKEN}"`
  networkID=`echo ${getNetworks} | jq -r '.[] | select(.name=="'${SITE_ID}' - '${ENV}' - DB") | .id'`
  if [[ ${groupID} != "" && ${networkID} != "" ]]; then
    mssqlJSON="{\"name\":\"mssql${1}\",\"description\":\"mssql server\",\"groupId\":\"${groupID}\",\"sourceServerId\":\"VA1TCCCSQLTMP01\",\"isManagedOS\":false,\"primaryDns\":\"172.17.1.26\",\"networkId\":\"${networkID}\",\"cpu\":1,\"memoryGB\":2,\"type\":\"standard\",\"storageType\":\"standard\",\"additionalDisks\":[{\"path\":\"E\",\"sizeGB\":30,\"type\":\"partitioned\"}]}"

    # Build Database
    build=`curl -s "https://${ENDPOINT}/v2/servers/${ACCT}" -XPUT -H "Authorization: Bearer ${TOKEN}" -XPOST -H "Content-type: application/json" -d "${mssqlJSON}"`
...
function updateJSONMssql {
  ...
  if [ -z "${server_found}" ]; then
    ...
    newJSON=`jq '(.Environments[] | select(.Name=="'${ENV}'") | .Servers['${total}']) |= { "Name" : "'${1}'","Type" : "MSSQL","Middleware" : "_NewServer_" }' ./${SITE_ID}.json`
    rm -rf ./${SITE_ID}.json
...
if [ ${found} -eq 0 ]; then
  if [ "${REF_ARCH}" == "Basic" ]; then
    basicRequestID=$(createMSSQLServer 1);
    if [ ${basicRequestID} -ne 0 ]; then
      echo "`date +"%c"`: Build mssql server on ${ACCT} has been queued. You will get an update every minute."
      ...
      while [ ${buildStatus} -ne 100 ]; do
        ...
        basicServer=`echo ${getStatus} | jq -r .Servers | grep MSSQL1 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
        echo "`date +"%c"`: Server ${basicServer} for ${ACCT} build is ${buildStatus}% complete..."
        timer=$(( timer + 1 ))
        if [ ${timer} == 20 ]; then
          break
        fi
      ...
      basicManagedID=$(makeManagedMSSQL ${basicServer});
      if [ ${basicManagedID} -ne 0 ]; then
        ...
        echo "`date +"%c"`: Server ${basicServer} is complete and will be ready for use in 4 minutes. Kicking off a 4 minute wait now so that sql modules can install before configure is run."
        wait 240
      ...
      echo "`date +"%c"`: Build mssql server failed to run.. Either the blueprint failed to finish or the network or group have not been created! Bailing..."
      exit 1
    fi

  elif [ "${REF_ARCH}" == "Standard" ]; then
    standardReqID1=$(createMSSQLServer 1);
    standardReqID2=$(createMSSQLServer 2);
    if [[ ${standardReqID1} -gt 0 && ${standardReqID2} -gt 0 ]]; then
      echo "`date +"%c"`: Build mssql standard servers on ${ACCT} have been queued. You will get an update every minute."
      ...
      while [ ${buildPercent1} -lt 100 ]; do
        while [ ${buildPercent2} -lt 100 ]; do
          ...
          standardServer1=`echo ${getBuildStatus1} | jq -r .Servers | grep MSSQL1 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
          standardServer2=`echo ${getBuildStatus2} | jq -r .Servers | grep MSSQL2 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
      ...
      standardManaged1=$(makeManagedMSSQL ${standardServer1});
      standardManaged2=$(makeManagedMSSQL ${standardServer2});
      if [[ ${standardManaged1} -gt 0 && ${standardManaged2} -gt 0 ]]; then
        ...
        # Update JSON
        updateJSONMssql ${standardServer1};
        updateJSONMssql ${standardServer2};
        ...
        echo "`date +"%c"`: Make Managed Blueprint for ${standardServer1} and ${standardServer2} failed to run. Go figure out why!"
      fi
    else
      echo "`date +"%c"`: Build standard mssql servers for ${ACCT} failed to run.. Either the blueprints failed to finish or the network or group have not been created! Bailing..."
      exit 1
    fi
  else
  ...
  if [ ${REF_ARCH} == "Basic" ]; then
    echo "`date +"%c"`: The required Basic ${ENV} Mssql Servers for ${ACCT} is built. Moving on..."
  elif [ ${REF_ARCH} == "Standard" ]; then
    basicRequestID=$(createMSSQLServer 2);
    if [ ${basicRequestID} -ne 0 ]; then
      echo "`date +"%c"`: Build mssql server on ${ACCT} has been queued. You will get an update every minute."
      ...
      while [ ${buildStatus} -ne 100 ]; do
        ...
        basicServer=`echo ${getStatus} | jq -r .Servers | grep MSSQL2 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
        echo "`date +"%c"`: Server ${basicServer} for ${ACCT} build is ${buildStatus}% complete..."
        timer=$(( timer + 1 ))
        if [ ${timer} == 20 ]; then
          break
  ...
  echo "`date +"%c"`: The required ${ENV} Mssql Servers for ${ACCT} are built. Moving on..."
fi
