...
if [ ${ACCT} == "" ]; then
  echo "`date +"%c"`: The ${ENV} Alias is missing from ${SITE_ID}.json. Mysql build cannot be completed without this information. Please ensure create subaccount finished successfully and updated the site's json."
  exit 1
fi
...
function getMysqlServers {
  # Check database total before creating any servers
...
function createMysqlServer {
  ...
  if [[ ${groupID} != "" && ${networkID} != "" ]]; then
    mysqlJSON="{\"name\":\"mysql${1}\",\"description\":\"database server\",\"groupId\":\"${groupID}\",\"sourceServerId\":\"VA1TCCCTMPL02\",\"isManagedOS\":false,\"primaryDns\":\"172.17.1.26\",\"networkId\":\"${networkID}\",\"cpu\":1,\"memoryGB\":2,\"type\":\"standard\",\"storageType\":\"standard\",\"Packages\":[{\"packageID\":\"24485861-e860-48bd-83c2-2bacb167f3ec\"}]}"

    # Build DB server
    build=`curl -s "https://${ENDPOINT}/v2/servers/${ACCT}" -XPUT -H "Authorization: Bearer ${TOKEN}" -XPOST -H "Content-type: application/json" -d "${mysqlJSON}"`
...
function updateJSONMysql {
  ...
  if [ -z "${server_found}" ]; then
    ...
    newJSON=`jq '(.Environments[] | select(.Name=="'${ENV}'") | .Servers['${total}']) |= { "Name" : "'${1}'","Type" : "db","Middleware" : "_NewServer_" }' ./${SITE_ID}.json`
    rm -rf ./${SITE_ID}.json
...
if [ ${found} -eq 0 ]; then
  if [ "${REF_ARCH}" == "Basic" ]; then
    basicRequestID=$(createMysqlServer 1);
    if [ ${basicRequestID} -ne 0 ]; then
      echo "`date +"%c"`: Build mysql server on ${ACCT} has been queued. You will get an update every 30 seconds."
      ...
      while [ ${basicBuildPercent} != 100 ]; do
        ...
        basicServer=`echo ${getStatus} | jq -r .Servers | grep MYSQL1 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
      ...
      echo "`date +"%c"`: Build mysql server failed to run.. Either the blueprint failed to run or the network or group have not been created!! Bailing..."
      exit 1
    fi

  elif [ "${REF_ARCH}" == "Standard" ]; then
    standardReqID1=$(createMysqlServer 1);
    standardReqID2=$(createMysqlServer 2);
    if [[ ${standardReqID1} -gt 0 && ${standardReqID2} -gt 0 ]]; then
      echo "`date +"%c"`: Build mysql standard servers on ${ACCT} have been queued. You will get an update every minute."
      ...
      while [ ${buildPercent1} -lt 100 ]; do
	    ...
	    getAuth;
          ...
          standardServer1=`echo ${getBuildStatus1} | jq -r .Servers | grep MYSQL1 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
          standardServer2=`echo ${getBuildStatus2} | jq -r .Servers | grep MYSQL2 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
	...
	if [ ${buildPercent1} -ne 100 ]; then
      ...
      # Update JSON
      updateJSONMysql ${standardServer1};
      updateJSONMysql ${standardServer2};
      ...
      echo "`date +"%c"`: Build standard mysql servers for ${ACCT} failed to run.. Either the blueprint failed to finish or the network or group have not been created!! Bailing..."
      exit 1
    fi
  else
  ...
  if [ ${REF_ARCH} == "Basic" ]; then
    echo "`date +"%c"`: The required Basic ${ENV} Mysql Servers for ${ACCT} is built. Moving on..."
  elif [ ${REF_ARCH} == "Standard" ]; then
    basicRequestID=$(createMysqlServer 2);
    if [ ${basicRequestID} -ne 0 ]; then
      echo "`date +"%c"`: Build mysql on ${ACCT} has been queued. You will get an update every 30 seconds."
	  ...
	  getAuth;
        ...
        basicServer=`echo ${getStatus} | jq -r .Servers | grep MYSQL2 | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
        echo "`date +"%c"`: Server ${basicServer} for ${ACCT} build is ${basicBuildPercent}% complete..."
        timer=$(( timer + 1 ))
        if [ ${timer} == 20 ]; then
          break
      ...
      echo "`date +"%c"`: Build mysql server failed to run.. Either the blueprint failed to finish or the network or group have not been created!! Bailing..."
      exit 1
    fi
  else
    echo "`date +"%c"`: No Reference Architecture was found for ${ACCT}! This should not have happened..."
  ...
  echo "`date +"%c"`: The required ${ENV} Mysql Servers for ${ACCT} are built. Moving on..."
fi
