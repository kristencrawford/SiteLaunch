...
function getMemcachedServers {
  # Check memcached total before creating any servers
...
function createMemDServer {
  getAuth;
  # Create build memcached server json
  ...
  groupID=`echo ${getGroups} | jq '.HardwareGroups[] | select(.Name=="'${SITE_ID}' - '${ENV}' - MEM") | .UUID' | sed "s/\"//g"`
  ...
  if [[ ${groupID} != "" && ${networkID} != "" ]]; then
    memDJSON="{\"name\":\"mche\",\"description\":\"memcached server\",\"groupId\":\"${groupID}\",\"sourceServerId\":\"VA1TCCCTMPL02\",\"isManagedOS\":false,\"primaryDns\":\"172.17.1.26\",\"networkId\":\"${networkID}\",\"cpu\":1,\"memoryGB\":8,\"type\":\"standard\",\"storageType\":\"standard\",\"Packages\":[{\"packageID\":\"24485861-e860-48bd-83c2-2bacb167f3ec\"}]}"

    # Build memcached server
    build=`curl -s "https://${ENDPOINT}/v2/servers/${ACCT}" -XPUT -H "Authorization: Bearer ${TOKEN}" -XPOST -H "Content-type: application/json" -d "${memDJSON}"`
    requestID=`echo ${build} | jq '.links[] | select(.rel=="status") | .id' | sed "s/\"//g" | awk -F- '{print $2}'`

    if [ ${requestID} -ne 0 ]; then
      echo "`date +"%c"`: Build memcached server on ${ACCT} has been queued. You will get an update every 30 seconds."
      ...
      while [ ${buildStatus} -ne 100 ]; do
        ...
        serverName=`echo ${getStatus} | jq -r .Servers | grep MCHE | sed "s/\"//g" | sed "s/\ //g" | sed "s/\,//g"`
      ...
      echo "`date +"%c"`: Build memcached server failed to run on ${ACCT}. Go figure out why. Bailing..."
      exit 1
    fi
  fi
}
...
function updateJSONmemD {
  ...
  if [ -z "${server_found}" ]; then
    ...
    newJSON=`jq '(.Environments[] | select(.Name=="'${ENV}'") | .Servers['${total}']) |= { "Name" : "'${1}'","Type" : "Memcached" }' ./${SITE_ID}.json`
    rm -rf ./${SITE_ID}.json
...
# Main
getAuth;
found=$(getMemcachedServers);
if [ ${found} -ne  0 ]; then
  echo "`date +"%c"`: All required ${ENV} Memcached Servers for ${ACCT} are built. Moving on..."
else
  createMemDServer;
  if [ "${?}" == "0" ]; then
